...
def main() -> int:
    ...
    """
    identifier, log_location, output_location, record_time, wifi_interface = (
        parse_arguments()
    )
    ...
    try:
        with GoPro(identifier, wifi_interface=wifi_interface) as gopro:
            ...
            media_set_before = set(
                x['n'] for x in gopro.wifi_command.get_media_list().flatten
            )
            # Take a video
            ...
            gopro.wifi_command.download_file(
                camera_file=video, local_file=output_location
            )
            console.print(
                f'Success!! :smiley: File has been downloaded to {output_location}'
            )
